...
export default function LeadSource() {
  ...
  const handleUpdate = (id) => {
    if (!editedName.trim()) {
      alert("Please enter lead source name");
      return;
    }
                ...
                <tr className="bg-gray-200 text-gray-800 font-medium">
                  <th className="border px-3 py-2">
                    <input
                      type="checkbox"
                      checked={
                        selected.length === filteredLeadSources.length && filteredLeadSources.length > 0
                      }
              ...
              <tbody>
                {filteredLeadSources.length === 0 ? (
                  ...
                  filteredLeadSources.map((lead, index) => (
                    <tr key={lead.id} className="text-center hover:bg-gray-50 transition">
